...
class UserAdsStorageManager : public StorageManager<UserAds>
{
public:
	UserAdsStorageManager(std::string filename) : StorageManager(filename) {}

	void saveToFile(const std::vector<std::shared_ptr<UserAds>>& objects) override {
		...
		for (const auto& obj : objects) {
			...
			unsigned int size = obj->ads().size();
			fwrite(&size, sizeof(unsigned int), 1, file);
			for (unsigned int i = 0; i < size; i++)
			{
				unsigned int ad_id = obj->ads()[i];
	...
	std::vector<std::shared_ptr<UserAds>> loadFromFile() const
	{
		...
		std::vector<std::shared_ptr<UserAds>> objects;
		...
		while (fread(&id, sizeof(unsigned int), 1, file) == 1) {
			...
			objects.push_back(std::make_shared<UserAds>(f));
		}
