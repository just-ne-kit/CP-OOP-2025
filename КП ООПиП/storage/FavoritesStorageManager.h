...
class FavoritesStorageManager : public StorageManager<FavoriteAds>
{
public:
	FavoritesStorageManager(std::string filename) : StorageManager(filename) {}

	void saveToFile(const std::vector<std::shared_ptr<FavoriteAds>>& objects) override {
	...
	std::vector<std::shared_ptr<FavoriteAds>> loadFromFile() const 
	{
		...
		std::vector<std::shared_ptr<FavoriteAds>> objects;
		...
		while (fread(&id, sizeof(unsigned int), 1, file) == 1) {
			...
			objects.push_back(std::make_shared<FavoriteAds>(f));
		}
